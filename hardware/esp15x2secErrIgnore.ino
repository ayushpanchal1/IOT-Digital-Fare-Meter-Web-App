...
void loop() {
   ...
   if (millis() - lastThingSpeakUpdate >= thingSpeakInterval) {
      ...
      // send data if valid data was received from uno
      if (dataReceived) {
   ...
   // ThingSpeak stats message using timer fix
   if (showStatusMessage && millis() - statusMessageStartTime >= statusDisplayTime) {
      showStatusMessage = false;  // clear this status message flag after duration
   }
}
